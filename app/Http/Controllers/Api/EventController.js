...
class EventController {
    ...
    * myEvents(req, res) {
        ...
        if (startDate.isValid() && endDate.isValid()) {
            ...
            }).populate('staffs').populate('employer', '_id name location locationName services type')
        ...
        if (req.user.isVenue) {
            ...
            if (req.user.employer._id.toString() === _event.employer._id.toString()) {
                ...
                return res.json({
                    ...
                    ids: [req.user.employer._id, _event.employer._id]
                })
            }
        ...
        if (req.user.isStaff) {
            if (_event.interested[req.user.staffId._id] == undefined) {
                ...
                if (_event.employer.interested[req.user.staffId._id] == undefined) {
                    ...
                    delete _event.employer.interested[req.user.staffId._id]
                    _event.employer.markModified('interested')
                    _event.employer.save()
                }
            } else {
                delete _event.interested[req.user.staffId._id]
                _event.markModified('interested')
                _event.save()
            }
